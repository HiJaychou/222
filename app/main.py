...
def protocols_page(request:Request):
    ...
    return templates.TemplateResponse('protocols.html',{'request':request,'active':'protocols','csrf_token':csrf_token(request),'protocols':protocols,'links':links,'singbox_version':system_ops.singbox_version(),'service_status':system_ops.service_status})
@app.post('/protocols/{protocol_key}/{action}')
def protocol_action(request:Request,protocol_key:str,action:str,csrf_token_value:str=Form(...,alias='csrf_token')):
    auth=require_login(request)
    if auth: return auth
...
@app.post('/users/add')
def users_add(request:Request,username:str=Form(...),protocol_key:str=Form(...),csrf_token_value:str=Form(...,alias='csrf_token')):
    auth=require_login(request)
    if auth: return auth
