...
def user_count(): con=connect(); count=con.execute('SELECT COUNT(*) FROM users').fetchone()[0]; con.close(); return count
...
def dashboard(request:Request):
    ...
    if auth: return auth
    protocols=protocol_rows(); status={p['protocol_key']:system_ops.service_status(p['service_name']) for p in protocols}; return templates.TemplateResponse('dashboard.html',{'request':request,'active':'home','csrf_token':csrf_token(request),'stats':system_ops.system_stats(),'net':system_ops.network_stats(),'protocols':protocols,'status':status,'installed':[p for p in protocols if p['installed']],'user_count':user_count(),'singbox_version':system_ops.singbox_version(),'open_ports':system_ops.list_open_ports()})
@app.get('/protocols')
...
def users_page(request:Request):
    auth=require_login(request)
    if auth: return auth
    con=connect(); users=con.execute('SELECT * FROM users ORDER BY id DESC').fetchall(); protocols=con.execute('SELECT * FROM protocols ORDER BY id').fetchall(); con.close(); return templates.TemplateResponse('users.html',{'request':request,'active':'users','csrf_token':csrf_token(request),'users':users,'protocols':protocols,'msg':request.query_params.get('msg',''),'error':request.query_params.get('error','')})
@app.post('/users/add')
...
def settings_page(request:Request):
    auth=require_login(request)
    if auth: return auth
    return templates.TemplateResponse('settings.html',{'request':request,'active':'settings','csrf_token':csrf_token(request),'panel_port':system_ops.panel_port(),'msg':request.query_params.get('msg',''),'error':request.query_params.get('error','')})
...
def uninstall_web(request:Request,csrf_token_value:str=Form(...,alias='csrf_token')):
    ...
    bad_csrf=csrf_redirect(request,csrf_token_value,'/settings')
    if bad_csrf: return bad_csrf
    ok,msg=system_ops.uninstall_web_panel(); log_action('uninstall web panel','iwantrun-vpn-web','ok' if ok else 'error',msg)
    return templates.TemplateResponse('settings.html',{'request':request,'active':'settings','csrf_token':csrf_token(request),'panel_port':system_ops.panel_port(),'msg':msg,'error':''})
...
def uninstall_all(request:Request,csrf_token_value:str=Form(...,alias='csrf_token')):
    ...
    bad_csrf=csrf_redirect(request,csrf_token_value,'/settings')
    if bad_csrf: return bad_csrf
    ok,msg=system_ops.uninstall_all_services(); log_action('uninstall all','all services','ok' if ok else 'error',msg)
    return templates.TemplateResponse('settings.html',{'request':request,'active':'settings','csrf_token':csrf_token(request),'panel_port':system_ops.panel_port(),'msg':msg,'error':''})
