...
def delete_user_from_protocol(key,username):
    ...
    c['inbounds'][0]['users']=new_users; checked_replace_config(key,c); _cmd(['systemctl','restart',SERVICES[key]],20); con=connect(); con.execute('DELETE FROM users WHERE username=? AND protocol_key=?',(username,key)); con.commit(); con.close()
