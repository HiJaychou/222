import json,secrets,socket,uuid,io
...
def install_protocol(key):
    ...
    ip=public_ip(); port=random_port(); username='user_'+secrets.token_hex(3); uid=str(uuid.uuid4()); password=secrets.token_hex(16); service=SERVICES[key]; extra={}
    if key=='vless-reality':
        priv,pub=reality_keys(); sid=secrets.token_hex(8); users=[{'name':username,'uuid':uid,'flow':'xtls-rprx-vision'}]
        ...
        priv,pub=reality_keys(); sid=secrets.token_hex(8); svc='grpc-service'; users=[{'name':username,'uuid':uid}]
        ...
        cert,k=cert_pair(proto_dir,'hysteria2',ip); users=[{'name':username,'password':password}]
        ...
        cert,k=cert_pair(proto_dir,'anytls',ip); users=[{'name':username,'password':password}]
        ...
        cert,k=cert_pair(proto_dir,'tuic',ip); users=[{'name':username,'uuid':uid,'password':password}]
    ...
    write_json(config_path,config); info={'protocol':key,'protocol_name':NAMES[key],'service':service,'server':ip,'port':port,'port_type':PORT_TYPES[key],'created_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),**extra}; write_json(info_path,info)
    _cmd([SB_BIN,'check','-c',str(config_path)],20); create_service(service,config_path); open_port(port,'udp' if PORT_TYPES[key]=='UDP' else 'tcp'); _cmd(['systemctl','restart',service],20)
...
def add_user_to_protocol(key,username):
    c=load_config(key)
    ...
    if key=='vless-reality': users.append({'name':username,'uuid':str(uuid.uuid4()),'flow':'xtls-rprx-vision'})
    elif key=='grpc-reality': users.append({'name':username,'uuid':str(uuid.uuid4())})
    elif key in ('hysteria2','anytls'): users.append({'name':username,'password':secrets.token_hex(16)})
    elif key=='tuic': users.append({'name':username,'uuid':str(uuid.uuid4()),'password':secrets.token_hex(16)})
    save_config(key,c); _cmd([SB_BIN,'check','-c',str(BASE/key/'config.json')],20); _cmd(['systemctl','restart',SERVICES[key]],20); con=connect(); con.execute('INSERT OR IGNORE INTO users(username,protocol_key,enabled,created_at) VALUES (?,?,1,?)',(username,key,datetime.now().strftime('%Y-%m-%d %H:%M:%S'))); con.commit(); con.close()
def delete_user_from_protocol(key,username):
    c=load_config(key)
    if not c: raise RuntimeError('协议未安装')
    users=c['inbounds'][0].get('users',[])
    if len(users)<=1: raise RuntimeError('不能删除最后一个用户')
    c['inbounds'][0]['users']=[u for u in users if u.get('name')!=username]; save_config(key,c); _cmd([SB_BIN,'check','-c',str(BASE/key/'config.json')],20); _cmd(['systemctl','restart',SERVICES[key]],20); con=connect(); con.execute('DELETE FROM users WHERE username=? AND protocol_key=?',(username,key)); con.commit(); con.close()
