...
print_result(){ echo_line; echo -e "${GREEN}自由档案馆｜VPN 管理面板安装完成${NC}"; echo_line; echo "访问地址：http://${SERVER_IP}:${PANEL_PORT}/${LOGIN_PATH}"; echo "管理员账号：admin"; echo "管理员密码：${ADMIN_PASS}"; echo "sing-box：$($SB_BIN version | head -n 1)"; echo -e "${YELLOW}请到 VPS 后台防火墙 / 安全组手动放行：${PANEL_PORT}/TCP${NC}"; }
main(){ check_root; echo_line; echo -e "${GREEN}自由档案馆 | iwantrun.com VPN Web Manager 安装脚本${NC}"; echo_line; install_dependencies; check_dependencies_ready; install_singbox_core; cleanup_old_install; download_project; install_python_env; init_settings_and_admin; create_service; open_firewall; get_server_ip; rm -rf "$TMP_DIR"; print_result; }
main "$@"
